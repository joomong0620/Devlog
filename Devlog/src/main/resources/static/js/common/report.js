function bindReportModalEvents() {
    ...
    reportButton?.addEventListener("click", () => {
        ...
        fetch("/report", {  // 저는 JPA 써서 할 예정이라 음 컨트롤러에서 타켓 타입으로 일단 분리해 둘게요 한 번 확인해 주세요
...
/* function openReportModal(targetMemberNo, targetNo) { <--------------------------- 함수 호출 시 타겟 회원 번호 넣어서 호출
    ...
    fetch(`/report/modal?memberNo=${targetMemberNo}`)  <------------------------------------------- 타켓 대상 회원 번호 넣어주셔야 합니다.
        ...
        modal.dataset.targetType = 'MESSAGE'; <-------------------------- 이 부분은 게시판이신 분들은 BOARD로 바꿔주세요
...
reOverlay?.addEventListener('click', e => {   <------------------------------모달 닫기용 입니다 . 
